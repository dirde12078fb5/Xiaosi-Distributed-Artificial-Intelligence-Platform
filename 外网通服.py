...
class VPNGui:
    ...
    def build_ui(self):
        tk.Label(self.root, text="外网通服", font=("微软雅黑", 18, "bold")).pack(pady=10)

        # 网卡选择
        frame_nic = ttk.LabelFrame(self.root, text="选择外网接口")
        frame_nic.pack(fill="x", padx=20, pady=5)
        ttk.Label(frame_nic, text="外网IP:").grid(row=0, column=0, padx=5, pady=5)
        ...
        ttk.Button(frame_nic, text="刷新接口", command=self.refresh_nic).grid(row=0, column=2, padx=5, pady=5)

        # IP 列表
        frame_ip = ttk.LabelFrame(self.root, text="外网IP地址")
        ...
        # 组网
        frame_p2p = ttk.LabelFrame(self.root, text="外网组网服务")
        ...
        # HTTP
        frame_http = ttk.LabelFrame(self.root, text="HTTP 外网服务")
        ...
        # HTTPS
        frame_https = ttk.LabelFrame(self.root, text="HTTPS 外网加密服务")
        ...
        # FTP
        frame_ftp = ttk.LabelFrame(self.root, text="FTP 外网文件服务")
        ...
        # 日志
        frame_log = ttk.LabelFrame(self.root, text="外网服务日志")
    ...
    def confirm_exit(self):
        if messagebox.askokcancel("确认退出", "确定要关闭外网通服吗？\n所有服务将停止运行。"):
    ...
    def start_https(self):
        ...
        def run():
            try:
                ...
                self.log(f"HTTPS外网服务启动 {ip}:{port}")
